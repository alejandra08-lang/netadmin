...
const obtener_ubicaciones = async (req, res) => {
    try { 
        const ubicaciones = await Ubicacion.obtener_ubicaciones();
...
const crear_ubicacion = async (req, res) => {
    ...
    if (!ubi_barrio || !ubi_localidad_municipio ||!ubi_tipo_via ||!ubi_numero ||!id_ciudad) {
    ...
    try { 
        const nuevaUbi = await Ubicacion.crear_ubicacion({ 
            ubi_barrio, 
            ubi_localidad_municipio,
            ubi_tipo_via, 
            ubi_numero, 
...
const eliminar_ubicacion = async (req, res) => {
    const { id } = req.params;
    try {
        const eliminada = await Ubicacion.eliminar_ubicacion(id);
